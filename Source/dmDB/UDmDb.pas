...
uses
  ...
  FireDAC.Comp.DataSet, FireDAC.Comp.Client,FMX.Dialogs,System.IOUtils,
...
type
  ...
  public
    ...
    function RetornaMaxRacao:string;
    ...
    function  VerificaTagEmUso(idCurral,Tipo,TagRFID:string):string;
    function  RetornaMaxIdFabricando(vIdFab: string): string;
  end;
...
begin
 ...
 begin
   ...
   Add('B.TAG_FIM,');
   Add('CAST(coalesce((select sum(KGREALIZADO) from Fornecimento s');
   Add('where s.data=a.data and S.IDRACAOPREV=A.IDRACAO AND S.IDCURRAL=A.IDCURRAL AND S.TRATO=A.TRATO');
   Add('),0)AS VARCHAR(20)) Realizado,');
   Add('(select min(s.horainicio) from Fornecimento s');
...
procedure Tudb.DeletaRacao;
begin
 with qryAux,qryAux.SQL do
 begin
   Clear;
   Add('DELETE FROM Racao');
   ExecSQL;

   Clear;
   Add('DELETE FROM RacaoAlimentos');
   ExecSQL;
 end;
end;
...
begin
 {$IF DEFINED(iOS) or DEFINED(ANDROID)}
  ...
  try
   vPath := TPath.Combine(TPath.GetDocumentsPath,'Trato4.db');
   if FileExists(vPath) then
    FCon.Params.Values['Database'] :=vPath
   else
    ShowMessage('Banco de dado nao localizado!');
 except
  on E: Exception do
 ...
 {$ENDIF}
 {$IFDEF MSWINDOWS}
   vPath := ExtractFilePath(ParamStr(0))+'db\Trato4W.db';
   if FileExists(vPath) then
   begin
    FCon.Params.DriverID :='SQLite';
    FCon.Params.Values['Database'] :=vPath;
...
var
 vLote:string;
begin
 vLote :=vIdUsuario+FormatDateTime('ddmmyyyyhhmmss',date);
 Fabricacao.Close;
 Fabricacao.Open;
 Fabricacao.Insert;
...
function Tudb.RetornaMaxIdFabricando(vIdFab: string): string;
var
 vQry:TFDQuery;
begin
 vQry:=TFDQuery.Create(nil);
 ...
 begin
   Clear;
   Add('select max(id) maxid from FabricacaoAlimentos');
   Add('where IdFabricacao='+vIdFab);
   Add('and RealizadoKG>0');
   Open;
   Result := vQry.FieldByName('maxid').Asstring;
 end;
 vQry.Free;
end;

function Tudb.RetornaMaxRacao: string;
var
 vQry:TFDQuery;
begin
 vQry:=TFDQuery.Create(nil);
 ...
 begin
   Clear;
   Add('select (max(id)+1) maxId from fabricacao');
   Open;
   Result := vQry.FieldByName('maxId').AsString;
 end;
 vQry.Free;
...
function Tudb.VerificaTagEmUso(idCurral,Tipo, TagRFID: string): String;
var
 vQry:TFDQuery;
begin
 vQry:=TFDQuery.Create(nil);
 ...
 begin
  ...
  begin
   ...
   Add('WHERE STATUS=1 AND ID<>'+idCurral+' AND TAG_INICIO='+TagRFID.QuotedString);
   Open;
   if not IsEmpty then
   begin
     Result       := 'Tag em uso no curral :'+FieldByName('Nome').AsString+' - Inicial';
     Exit;
   end;
  end;
  ...
  begin
   Clear;
   Add('SELECT * FROM Currais');
   Add('WHERE STATUS=1 AND ID<>'+idCurral+' AND TAG_FIM='+TagRFID.QuotedString);
   Open;
   if not IsEmpty then
   begin
     Result       := 'Tag em uso no curral :'+FieldByName('Nome').AsString+' - Final';
     Exit;
   end;
  end;
  Result :='OK';
